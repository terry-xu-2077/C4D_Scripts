...
用于批量给材质管理器中当前选中的 Cinema 4D 标准/默认渲染器材质分配不重复的随机彩色。

主要行为：

- 仅处理标准材质 `Mmaterial`，Redshift、节点材质等其它材质类型自动跳过。
- 自动启用并修改材质的 **颜色（Color）** 通道。
- 不删除颜色贴图，也不修改反射、透明、凹凸等其它材质参数。
- 颜色采用“随机起始色相 + 均匀色相分布 + 随机饱和度/明度”，保证同次执行的颜色不重复，并尽量避免颜色彼此过近。
- 支持 Cinema 4D Undo，可直接撤销整次批量修改。

使用时在材质管理器中多选需要处理的标准材质，然后运行脚本即可；再次运行会重新生成另一组随机配色。
